...
const Task = () =>{
    ...
    useEffect(()=>{
        window.scrollTo(0, 0);
        axios.get(`http://localhost:3000/api/users/${userId}/tasks/${taskId}`, { headers: { "Authorization": `Bearer ${jwtToken}`} })
            .then((res)=>{
        ...
        axios.get(`http://localhost:3000/api/users/${userId}/tasks/${taskId}/comments`, { headers: { "Authorization": `Bearer ${jwtToken}`} })
            .then((res)=>{
