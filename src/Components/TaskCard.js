...
const TaskCard = (props) =>{
    ...
    function handleStatus(e) {
        ...
        if (userId == assignee_id) {
            axios.post(`http://localhost:3000/tasks/${taskId}/edit`, {status: !taskStatus}, {headers: {"Authorization": `Bearer ${jwtToken}`}})
                .then((res) => {
                    console.log('Done')
                })
                .catch((err) => {
    ...
    function handleDelete(e){
        e.preventDefault()
        axios.delete(`http://localhost:3000/users/${userId}/tasks/${taskId}`, { headers: { "Authorization": `Bearer ${jwtToken}`} })
            .then((res)=>{
                console.log('Sanjay')
            })
            .catch((err)=>{
